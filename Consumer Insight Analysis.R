...
# Reordered the membership type  and satisfaction level column to make them ordinal

df$membership_type <- factor(df$membership_type, 
...
df$satisfaction_level <- factor(df$satisfaction_level,
                                levels = c("Unsatisfied", 
                                           "Neutral", 
                                           "Satisfied"),
                                labels = c("Unsatisfied", 
                                           "Neutral", 
                                           "Satisfied"),
                                ordered = TRUE)
...
plot_frq(df$age_category,
  ...
  xlab("Age Category") +
  ylab("Count") +
  labs(title = "Consumers' Age Category")
...
plot_frq(df$gender,
  ...
  xlab("Gender") +
  ylab("Count") +
  labs(title = "Consumers' Gender Identity")
...
plot_frq(df$membership_type,
  ...
  ylab("Count") +
  labs(title = "Consumers' Membership Type")
...
plot_frq(df$satisfaction_level,
  ...
  ylab("Count") +
  labs(title = "Consumers' Level of Satisfaction")
...
# Levene's Test for Equal Variances (Median)

library(rstatix)

equal_var_test <- levene_test(data = df, total_spend ~ age_category,
                              center = "median")


equal_var_test <- equal_var_test %>%
...
# Levene's Test for Equal Variances (Mean)

equal_var_test_t_test <- levene_test(data = df, 
                                     total_spend ~ discount_applied,
                                     center = "mean")


equal_var_test_t_test <- equal_var_test_t_test %>%
  mutate(statistic = round(statistic, 2),
         p = case_when(
           p <0.001 ~ "<0.001"
         ))

print(equal_var_test_t_test)
